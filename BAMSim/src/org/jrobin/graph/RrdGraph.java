		...
		for (PlotElement plotElement : gdef.plotElements) {
			if (plotElement instanceof SourcedPlotElement) {
				...
				if (source instanceof Line) {
					...
					worker.drawPolyline(x, y, source.color, ((Line) source).getBasicStroke());
				}
			...
			for (CommentText c : gdef.comments) {
				if (c.isValidGraphElement()) {
					...
					if (c instanceof LegendText) {
						// draw with BOX
						if(((LegendText) c).getBasicStroke()==null)
						{
						...
						{
							worker.drawLine(x, y-3, x+15, y-3, ((LegendText) c).legendColor, ((LegendText) c).getBasicStroke());
						}
						worker.drawString(c.resolvedText, x + boxSpace+5, y, gdef.smallFont, gdef.colors[COLOR_FONT]);
					}
					else {
